...
void
TerrainVehicleSprite::turnleft() {
    vehicle.setFaceAngle(vehicle.getFaceAngle() - turnrate);
    if (vehicle.getFaceAngle() < 0)
        vehicle.setFaceAngle(359);
}

void
TerrainVehicleSprite::turnright() {
    vehicle.setFaceAngle(vehicle.getFaceAngle() + turnrate);
    if (vehicle.getFaceAngle() > 359)
        vehicle.setFaceAngle(0);
}

void
TerrainVehicleSprite::limitvelocity() {
    if (vehicle.getVelX() > 3.0)
        vehicle.setVelX(3.0);
    else if (vehicle.getVelX() < -3.0)
        vehicle.setVelX(-3.0);
    if (vehicle.getVelY() > 3.0)
        vehicle.setVelY(3.0);
    else if (vehicle.getVelY() < -3.0)
        vehicle.setVelY(-3.0);
}

void
TerrainVehicleSprite::applythrust() {
...
void
TerrainVehicleSprite::cruise() {
    vehicle.setCurrFrame(0);
}

void
TerrainVehicleSprite::allstop() {
    vehicle.setVelX(0.0);
    vehicle.setVelY(0.0);
}
...
void
TerrainVehicleSprite::applybraking() {
...
void
TerrainVehicleSprite::draw(ALLEGRO_BITMAP *dest) {
    vehicle.drawframe_rotate(dest, vehicle.getFaceAngle());
}

float
TerrainVehicleSprite::getVelocityX() {
    return vehicle.getVelX();
}

float
TerrainVehicleSprite::getVelocityY() {
    return vehicle.getVelY();
}

float
TerrainVehicleSprite::getRotationAngle() {
    return vehicle.getFaceAngle();
}

void
TerrainVehicleSprite::reset() {
    allstop();
    vehicle.setCurrFrame(0);
}
