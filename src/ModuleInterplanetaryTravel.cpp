...
ModuleInterPlanetaryTravel::on_event(ALLEGRO_EVENT *event) {
    Ship ship;
    std::string escape = "";
    ALLEGRO_EVENT complete = {.type = EVENT_COMMAND_COMPLETE};
    ...
    case EVENT_CAPTAIN_LAUNCH:
        g_game->printout(
            ...
            BLUE,
            8000);
        g_game->broadcast_event(&complete);
        break;
    ...
    case EVENT_TACTICAL_SHIELDS:
        g_game->printout(
            OFFICER_TACTICAL, m_text, "Sir, we are not in combat.", BLUE, 8000);
        g_game->broadcast_event(&complete);
        break;
...
ModuleInterPlanetaryTravel::on_update() {
    ...
    // getting too close to the star?
        ...
        if (burning > 240) {
            ...
            g_game->printout(m_text, "AAARRRRRGGGGHHHHH!!!", YELLOW, 500);
            ship->allstop();
    ...
    if (flag_DoOrbit) {
        ...
        if (!planet) {
            ...
            g_game->printout(
                OFFICER_NAVIGATION, m_text, "Aye, captain.", ORANGE, 8000);
