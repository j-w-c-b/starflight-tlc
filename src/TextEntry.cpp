...
TextEntry::TextEntry(
    ...
    const string &prompt,
    int max_chars,
    shared_ptr<ALLEGRO_BITMAP> cursor,
    float cursor_display_time,
    int x,
    int y,
    int width,
    int height,
    shared_ptr<Sample> click_sound,
    shared_ptr<Sample> error_sound,
...
TextEntry::on_key_pressed(ALLEGRO_KEYBOARD_EVENT *event) {
    ...
    if (event->keycode == ALLEGRO_KEY_BACKSPACE) {
        if (m_text.size() > 0) {
            ...
            if (m_cursor) {
                m_cursor->move(
                    get_x() + m_text_display->get_text_width(), get_y());
            }
        } else {
            play_err = true;
        }
...
TextEntry::set_prompt(const std::string &prompt) {
    ...
    if (m_cursor) {
        m_cursor->move(get_x() + m_text_display->get_text_width(), get_y());
    }
}
...
void
TextEntry::set_text(const string &text) {
    m_text = text;
    m_text_display->set_text(m_prompt_text + m_text);
    if (m_cursor) {
        m_cursor->move(get_x() + m_text_display->get_text_width(), get_y());
