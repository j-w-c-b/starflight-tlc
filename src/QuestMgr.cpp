...
void
QuestMgr::setStoredValue(int value) {
    g_game->gameState->setStoredValue(value);
}

void
QuestMgr::raiseEvent(int eventid, int param1, int param2) {
...
QuestMgr::VerifyRequirements(int reqCode, int reqType, int reqAmt) {
    ...
    case 10:
        starSystem = reqType;
        if (starSystem == storedValue)
            req = true;
        break;

    // 12 = orbit planet
    case 12:
        if (reqType == storedValue)
            req = true;
        break;
...
QuestMgr::giveReward() {
    ...
    case 2:
        ...
        if (!item->IsArtifact()) {
            ...
            if (freeSpace <= 0) {
                reward = "You received nothing. Remember to make free space in "
                         "your hold next time!";
                break;
            }
    ...
    /**
            reset stored value
        Note: this is crucial to the functioning of the QuestMgr since
