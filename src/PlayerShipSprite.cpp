...
PlayerShipSprite::PlayerShipSprite() : ship() {
    // initialize and load the ship sprite
    switch (g_game->gameState->getProfession()) {
    case PROFESSION_FREELANCE:
        ship.setImage(images[spacetravel::I_SHIP_FREELANCE]);
        break;

    case PROFESSION_MILITARY:
        ship.setImage(images[spacetravel::I_SHIP_MILITARY]);
        break;

    case PROFESSION_SCIENTIFIC:
    default:
        ship.setImage(images[spacetravel::I_SHIP_SCIENCE]);
        break;
    }
    if (!ship.getImage()) {
...
double
PlayerShipSprite::getMaximumVelocity() {
    double topspeed = 1.0;
...
double
PlayerShipSprite::getForwardThrust() {
    double accel = 1.0;

    int engine = g_game->gameState->getShip().getEngineClass();
    if (engine < 1 || engine > 6) {
        engine = 1;
        ALLEGRO_DEBUG("*** Error in PlayerShipSprite::getForwardThrust: Engine "
                      "class is invalid");
    }

    switch (engine) {
    case 1:
        accel = g_game->getGlobalNumber("ENGINE1_ACCEL");
        break;
    case 2:
        accel = g_game->getGlobalNumber("ENGINE2_ACCEL");
        break;
    case 3:
        accel = g_game->getGlobalNumber("ENGINE3_ACCEL");
        break;
    case 4:
        accel = g_game->getGlobalNumber("ENGINE4_ACCEL");
        break;
    case 5:
        accel = g_game->getGlobalNumber("ENGINE5_ACCEL");
        break;
    case 6:
        accel = g_game->getGlobalNumber("ENGINE6_ACCEL");
        break;
    }
...
double
PlayerShipSprite::getReverseThrust() {
    return getForwardThrust() * 0.5;
}

// Read lateral thrust from script
double
PlayerShipSprite::getLateralThrust() {
    return getForwardThrust() * 0.5;
}

// Read turning rate from script
double
PlayerShipSprite::getTurnRate() {
    double turnrate = 1.0;

    int engine = g_game->gameState->getShip().getEngineClass();
    if (engine < 1 || engine > 6) {
        engine = 1;
        ALLEGRO_DEBUG("*** Error in PlayerShipSprite::getTurnRate: Engine "
                      "class is invalid");
    }

    switch (engine) {
    case 1:
        turnrate = g_game->getGlobalNumber("ENGINE1_TURNRATE");
        break;
    case 2:
        turnrate = g_game->getGlobalNumber("ENGINE2_TURNRATE");
        break;
    case 3:
        turnrate = g_game->getGlobalNumber("ENGINE3_TURNRATE");
        break;
    case 4:
        turnrate = g_game->getGlobalNumber("ENGINE4_TURNRATE");
        break;
    case 5:
        turnrate = g_game->getGlobalNumber("ENGINE5_TURNRATE");
        break;
    case 6:
        turnrate = g_game->getGlobalNumber("ENGINE6_TURNRATE");
        break;
    }
...
void
PlayerShipSprite::limitvelocity() {
    double vx = 0;
    double vy = 0;
...
void
PlayerShipSprite::cruise() {
    ship.setCurrFrame(0);
}

void
PlayerShipSprite::allstop() {
    ship.setVelX(0.0);
    ship.setVelY(0.0);
    ship.setCurrFrame(0);
}
...
void
PlayerShipSprite::applybraking() {
    // slow down ship braking to 60 fps
    if (!braking_timer.stopwatch(TIMER_RATE))
        return;
...
void
PlayerShipSprite::draw(ALLEGRO_BITMAP *dest) {
    ship.drawframe_rotate(dest, (int)ship.getFaceAngle());
}

float
PlayerShipSprite::getVelocityX() {
    return ship.getVelX();
}

float
PlayerShipSprite::getVelocityY() {
    return ship.getVelY();
}

void
PlayerShipSprite::setVelocityX(float value) {
    ship.setVelX(value);
}

void
PlayerShipSprite::setVelocityY(float value) {
    ship.setVelY(value);
}

float
PlayerShipSprite::getRotationAngle() {
    return ship.getFaceAngle();
}

void
PlayerShipSprite::reset() {
    allstop();
    ship.setCurrFrame(0);
}

double
PlayerShipSprite::getCurrentSpeed() {
