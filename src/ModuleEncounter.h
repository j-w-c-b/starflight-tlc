...
class ModuleEncounter : public Module {
  private:
    static constexpr int NormalScreenHeight = 512;
    static constexpr int FullScreenHeight = SCREEN_HEIGHT;
    enum PostureStates
    {
        POSTURE_NONE = 0,
        POSTURE_OBSEQUIOUS,
    ...
    std::shared_ptr<Bitmap> m_portrait_view;
    std::shared_ptr<Bitmap> m_ship_view;
  ...
  public:
    ...
    std::vector<std::unique_ptr<CombatObject>> combatObjects;
    std::shared_ptr<TileScroller> m_scroller;
    std::shared_ptr<Bitmap> m_minimap;
    std::shared_ptr<Bitmap> m_encounter_display;
    std::shared_ptr<Bitmap> m_aux_gui;

    void AddCombatObject(CombatObject *CObject);

    std::map<std::string, std::string> dialogCensor;

    std::shared_ptr<Sample> m_fire1;
    std::shared_ptr<Sample> m_fire2;
    std::shared_ptr<Sample> m_hit1;
    std::shared_ptr<Sample> m_hit2;
};
