...
CombatObject::TimedUpdate() {
    ...
    if (this->rotation != 0) {
        ...
        if (this->getFaceAngle() < 0.0)
            this->setFaceAngle(360 - this->getFaceAngle());
        if (this->getFaceAngle() > 359.0)
            this->setFaceAngle(0);
    }
}

bool
CombatObject::CheckCollision(CombatObject *other) {
    /*JH 09/08
      there is a strange bug in the BR collision function that ignores collision
      if first sprite is larger than the second one. need a more  generic
      Rect-based collision routine. for combat let's just stick with
...
void
CombatObject::ApplyImpact(CombatObject *incoming) {
...
void
CombatObject::TurnRight() {
    this->setFaceAngle(this->getFaceAngle() + turnrate);
    if (this->getFaceAngle() > 359)
        this->setFaceAngle(360 - this->getFaceAngle());
}

double
CombatObject::getRelativeSpeed() {
