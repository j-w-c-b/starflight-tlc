...
ModulePlanetOrbit::OnEvent(Event *event) {
    ...
    case EVENT_SCIENCE_ANALYSIS:
        if (planetScan == 3 && planetAnalysis == 0) {
            g_game->printout(
                text,
                sci + "Affirmative, Captain. Analyzing sensor data...",
                LTGREEN,
                2000);
            analyzeplanet();
        }
    ...
    case EVENT_NAVIGATOR_DOCK:
        ...
        if (g_game->gameState->player->currentPlanet == 8) {
            g_game->printout(
                text, nav + "Yes, sir! Docking with Starport.", ORANGE, 5000);
            flag_DoDock = true;
        } else {
            g_game->printout(
                text,
    ...
    case EVENT_NAVIGATOR_ORBIT:
        g_game->printout(
            text, nav + "We are already orbiting a planet!", YELLOW, -1);
        break;
    ...
    case EVENT_COMM_HAIL:
        g_game->printout(
            text, com + "There are no other ships in the vicinity", YELLOW, -1);
        break;
    ...
    case EVENT_COMM_QUESTION:
        g_game->printout(
            text, com + "We are not in contact with anyone", YELLOW, -1);
        break;
    ...
    case EVENT_COMM_TERMINATE:
        g_game->printout(
            text, com + "We are not engaged in communication", YELLOW, -1);
        break;

    case EVENT_DOCTOR_EXAMINE:
        g_game->printout(
            text,
            doc + "Please break orbit to access the crew's medical status",
            YELLOW,
            -1);
        break;
    ...
    case EVENT_DOCTOR_TREAT:
        g_game->printout(
            text,
            doc + "Please break orbit to set crew treatment priorities",
            YELLOW,
            -1);
        break;
    }
...
ModulePlanetOrbit::CreatePlanetTexture() {
    ...
    // planet textures are stored in data/planetorbit using the starid and
    // planetid as a random seed once created they are re-used two textures are
    // required for each planet as a result of replacing the OpenGL with a
    // software renderer
    ...
    if (!planet_texture) {
        ...
        planet_texture =
            (ALLEGRO_BITMAP *)al_load_bitmap(orbitFilename.c_str());
        if (!planet_texture) {
...
ModulePlanetOrbit::Init() {
    ...
    if (g_game->getGlobalBoolean("AUDIO_GLOBAL")) {
        audio_scan =
            g_game->audioSystem->Load("data/spacetravel/audio_scan.wav");
        if (!audio_scan) {
...
ModulePlanetOrbit::Update() {
    ...
    if (planetScan == 1) {
        ...
        g_game->printout(
            text, sci + "Sensor analysis is complete.", WHITE, 1000);
    ...
    if (planetScan == 1) {
        ...
        for (int n = 0; n < 8; n++) {
            ...
            r.bottom = r.top + 4;
            ALLEGRO_COLOR color =
                al_map_rgb(100 + rand() % 155, 0, 100 + rand() % 155);
...
ModulePlanetOrbit::Draw() {
    ...
    al_draw_bitmap(
        lightmap_overlay, cx + lightmapOffsetX, cy + lightmapOffsetY, 0);
