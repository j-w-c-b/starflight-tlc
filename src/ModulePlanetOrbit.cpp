...
ModulePlanetOrbit::on_event(ALLEGRO_EVENT *event) {
    ...
    case EVENT_TACTICAL_SHIELDS:
        g_game->printout(
            OFFICER_TACTICAL, m_text, "We are not in combat", YELLOW, -1);
        break;
    ...
    case EVENT_ENGINEER_REPAIR:
        g_game->printout(
            OFFICER_ENGINEER,
            m_text,
            "Please break orbit to access damage control",
            YELLOW,
            -1);
        break;
    ...
    case EVENT_DOCTOR_EXAMINE:
        g_game->printout(
            OFFICER_MEDICAL,
            m_text,
            "Please break orbit to access the crew's medical status",
            YELLOW,
            -1);
        break;
    ...
    case EVENT_DOCTOR_TREAT:
        g_game->printout(
            OFFICER_MEDICAL,
            m_text,
            "Please break orbit to set crew treatment priorities",
            YELLOW,
            -1);
        break;
    }
...
ModulePlanetOrbit::on_update() {
    ...
    if (planetScan == 1) {
        ...
        ALLEGRO_EVENT e{.type = EVENT_COMMAND_COMPLETE};
        ...
        g_game->printout(
            ...
            WHITE,
            1000);
        g_game->printout(m_text, temp, STEEL, 1000);
        ...
        for (int n = 0; n < g_game->dataMgr->GetNumItems(); n++) {
                ...
                || item->itemType == IT_RUIN) // jjh
            {
                ...
                if (item->planetid == planetid) {
                    ...
                    g_game->printout(
                        OFFICER_SCIENCE, m_text, os.str(), YELLOW, 1000);
        ...
        g_game->printout(
            OFFICER_NAVIGATION, m_text, "Orbit established.", LTGREEN, -1);
        switch (planet->size) {
        case PS_SMALL:
        ...
        g_game->printout(OFFICER_SCIENCE, m_text, temp, ORANGE, -1);
    }
...
ModulePlanetOrbit::on_draw(ALLEGRO_BITMAP *target) {
    ...
    al_draw_bitmap(images[I_STARFIELD].get(), 0, 0, 0);
