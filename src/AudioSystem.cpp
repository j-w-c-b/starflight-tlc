...
bool Sample::SetVolume(float volume)
{
    ...
    if (volume < 0.0f || volume > 10.0f) return false;
 ...
 */
bool Sample::SetLoop(bool doLoop)
{
	loop = doLoop;
	...
	if (loop) {
		return al_set_sample_instance_playmode(sample_instance, ALLEGRO_PLAYMODE_LOOP);
	}
	else {
		return al_set_sample_instance_playmode(sample_instance, ALLEGRO_PLAYMODE_ONCE);
	}
}
 ...
 */
bool Sample::SetPaused(bool doPause)
{
	ALLEGRO_ASSERT(sample_instance != NULL);
...
shared_ptr<Sample> AudioSystem::Load(const string &filename, float volume)
{
...
bool AudioSystem::Load(const string &filename, const string &name, float volume)
{
...
shared_ptr<Sample> AudioSystem::LoadMusic(const string &filename, float volume)
{
	return Load(filename, volume);
}

bool AudioSystem::LoadMusic(const string &filename, const string &name, float volume)
{
	return Load(filename, name, volume);
}

bool AudioSystem::SampleExists(const string &name)
{
	auto i = samples.find(name);
	return i != samples.end();
}

bool AudioSystem::IsPlaying(const string &name)
{
	auto i = samples.find(name);
	if (i == samples.end())
	{
...
bool AudioSystem::IsPlaying(shared_ptr<Sample> sample)
{
	return sample->IsPlaying();
}


bool AudioSystem::Play(const string &name, bool doLoop)
{
...
bool AudioSystem::Play(shared_ptr<Sample> sample, bool doLoop)
{
	//prevent playback based on global setting
	if (!bPlay) return true;

	return sample && sample->IsInitialized() && sample->SetLoop(doLoop) && sample->SetPaused(false);
}

bool AudioSystem::PlayMusic(const string &name, bool doLoop)
{
	return Play(name, doLoop);
}

bool AudioSystem::PlayMusic(shared_ptr<Sample> sample, bool doLoop)
{
	return Play(sample, doLoop);
}

void AudioSystem::Stop(const string &name)
{
	auto i = samples.find(name);
	if (i == samples.end()) return;

	Stop(i->second);
}

void AudioSystem::Stop(shared_ptr<Sample> sample)
{
