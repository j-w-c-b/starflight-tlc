...
template <class T>
ResourceManager<T>::ResourceManager(const ResourceName resource_names[]) : resources()
{
...
ResourceManager<ALLEGRO_BITMAP>::load(Resource<ALLEGRO_BITMAP> &data) {
    ...
    resources[data.name].data = std::unique_ptr<ALLEGRO_BITMAP, void (*)(ALLEGRO_BITMAP*)>(b, Resource<ALLEGRO_BITMAP>::deleter);
    return b;
}
