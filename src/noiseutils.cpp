...
inline noise::uint8
BlendChannel(const uint8 channel0, const uint8 channel1, float alpha) {
...
// hope.)
inline noise::uint8 *
UnpackFloat(noise::uint8 *bytes, float value) {
...
inline noise::uint8 *
UnpackLittle16(noise::uint8 *bytes, noise::uint16 integer) {
...
inline noise::uint8 *
UnpackLittle32(noise::uint8 *bytes, noise::uint32 integer) {
...
GradientColor::GradientColor() { m_pGradientPoints = NULL; }
...
int
GradientColor::FindInsertionPos(double gradientPos) {
    int insertionPos;
...
const Color &
GradientColor::GetColor(double gradientPos) const {
...
void
NoiseMap::CopyNoiseMap(const NoiseMap &source) {
...
void
NoiseMap::DeleteNoiseMapAndReset() {
    delete[] m_pNoiseMap;
    InitObj();
}

float
NoiseMap::GetValue(int x, int y) const {
    if (m_pNoiseMap != NULL) {
...
NoiseMap::SetSize(int width, int height) {
    if (width < 0 || height < 0 || width > RASTER_MAX_WIDTH ||
        ...
        if (m_memUsed < newMemUsage) {
            // The new size is too big for the current noise map buffer.  We
            // need to reallocate.
            DeleteNoiseMapAndReset();
            try {
...
void
NoiseMap::SetValue(int x, int y, float value) {
    if (m_pNoiseMap != NULL) {
        if (x >= 0 && x < m_width && y >= 0 && y < m_height) {
...
void
Image::DeleteImageAndReset() {
    delete[] m_pImage;
    InitObj();
}

Color
Image::GetValue(int x, int y) const {
    if (m_pImage != NULL) {
        if (x >= 0 && x < m_width && y >= 0 && y < m_height) {
            return *(GetConstSlabPtr(x, y));
        }
...
void
Image::SetValue(int x, int y, const Color &value) {
    if (m_pImage != NULL) {
        if (x >= 0 && x < m_width && y >= 0 && y < m_height) {
            *(GetSlabPtr(x, y)) = value;
        }
...
int
WriterBMP::CalcWidthByteCount(int width) const {
...
int
WriterTER::CalcWidthByteCount(int width) const {
...
NoiseMapBuilder::NoiseMapBuilder()
    : m_pCallback(NULL), m_destHeight(0), m_destWidth(0), m_pDestNoiseMap(NULL),
      m_pSourceModule(NULL) {}

void
NoiseMapBuilder::SetCallback(NoiseMapCallback pCallback) {
...
NoiseMapBuilderCylinder::NoiseMapBuilderCylinder()
    : m_lowerAngleBound(0.0), m_lowerHeightBound(0.0), m_upperAngleBound(0.0),
...
NoiseMapBuilderPlane::NoiseMapBuilderPlane()
    : m_isSeamlessEnabled(false), m_lowerXBound(0.0), m_lowerZBound(0.0),
      m_upperXBound(0.0), m_upperZBound(0.0) {}
...
NoiseMapBuilderSphere::NoiseMapBuilderSphere()
    : m_eastLonBound(0.0), m_northLatBound(0.0), m_southLatBound(0.0),
      m_westLonBound(0.0) {}
...
RendererImage::RendererImage()
    : m_isLightEnabled(false), m_isWrapEnabled(false), m_lightAzimuth(45.0),
      m_lightBrightness(1.0), m_lightColor(255, 255, 255, 255),
      m_lightContrast(1.0), m_lightElev(45.0), m_lightIntensity(1.0),
      m_pBackgroundImage(NULL), m_pDestImage(NULL), m_pSourceNoiseMap(NULL),
...
RendererImage::CalcLightIntensity(
    ...
    double ix =
        (I_MAX - io) * m_lightContrast * SQRT_2 * m_cosElev * m_cosAzimuth;
    double iy =
        (I_MAX - io) * m_lightContrast * SQRT_2 * m_cosElev * m_sinAzimuth;
...
RendererImage::Render() {
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            // If lighting is enabled, calculate the light intensity based on
            // the rate of change at the current point in the noise map.
            ...
            if (m_isLightEnabled) {
                ...
                // Get the noise value of the current point in the source noise
                // map and the noise values of its four-neighbors.
...
RendererNormalMap::RendererNormalMap()
    : m_bumpHeight(1.0), m_isWrapEnabled(false), m_pDestImage(NULL),
