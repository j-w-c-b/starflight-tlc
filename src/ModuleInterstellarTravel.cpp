...
// scroller properties
static const int GALAXY_SCROLL_X = 0;
static const int GALAXY_SCROLL_Y = 0;
static const int GALAXY_SCROLL_WIDTH = SCREEN_WIDTH;
static const int GALAXY_SCROLL_HEIGHT = 650;
static const int GALAXYTILESIZE = 128;
static const int GALAXYTILESACROSS = 250;
static const int GALAXYTILESDOWN = 220;
...
ModuleInterstellarTravel::on_event(ALLEGRO_EVENT *event) {
    ...
    case EVENT_TACTICAL_SHIELDS:
        g_game->printout(
            OFFICER_TACTICAL, "Sir, we are not in combat.", BLUE, 2000);
        break;
...
ModuleInterstellarTravel::RollEncounter(AlienRaces forceThisRace) {
    ...
    if (forcedRace != ALIEN_NONE) {
        ...
        g_game->printout(OFFICER_NAVIGATION, os.str(), BRTORANGE, 30000);

        os.str("");
        os << "(muttered) Is that even a sentence?";
        g_game->printout(OFFICER_COMMUNICATION, os.str(), ROYALBLUE, 30000);

        os.str("");
        os << "Captain we seem to be in " << alienRaceText
           << " space. And they've found us!";
        g_game->printout(OFFICER_NAVIGATION, os.str(), KHAKI, 30000);

        g_game->audioSystem->Play(m_klaxon);
        g_game->printout(OFFICER_SCIENCE, "Encounter Alert!", RED, 5000);
...

    {
        ...
        if (alienRace != lastAlien) {
            ...
            g_game->printout(OFFICER_NAVIGATION, os.str(), GREEN, 30000);
            lastAlien = alienRace;
        } else {
            if (alienRace == ALIEN_PIRATE) {
                os << "Captain, we must be cautious as many freelancers and "
                      "pirates frequent this area of space.";
                g_game->printout(OFFICER_SCIENCE, os.str(), BRTORANGE, -1);
            } else {
                os << "Captain, we are " << depth << " " << alienRaceText
                   << " space.";
                g_game->printout(
                    OFFICER_NAVIGATION, os.str(), BRTORANGE, 30000);
            }
        }
    }
    ...
    if (movement_counter > 0) {
        ...
        case 2: // second "hit" generates size of fleet
            ...
            g_game->printout(OFFICER_SCIENCE, os.str(), YELLOW, 8000);
            break;
        ...
        case 3: // third "hit" could randomly reset the counter
            ...
            if (attitude > 90) {
                ...
                g_game->printout(OFFICER_COMMUNICATION, os.str(), GREEN, 8000);
            } else if (attitude > 74) {
                ...
                g_game->printout(OFFICER_COMMUNICATION, os.str(), YELLOW, 8000);
            } else {
...
ModuleInterstellarTravel::on_update() {
    ...
    if (flag_launchEncounter) {
        ...
        if (!g_game->audioSystem->IsPlaying(m_klaxon)) {
    ...
    if (starFound) {
        ...
        g_game->printout(OFFICER_NAVIGATION, os.str(), ORANGE, 20000);
    ...
    if (g_game->gameState->player.isLost()) {
        g_game->printout(
            ...
            ORANGE,
            8000);
        g_game->printout(OFFICER_CAPTAIN, "...Oh no.", LTGREEN, 4000);
        ...
        if (g_game->gameState->CanSkillCheck(SKILL_NAVIGATION)
            ...
            g_game->printout(
                ...
                ORANGE,
                8000);
            g_game->printout(OFFICER_CAPTAIN, "...", LTGREEN, 1000);
            flux = nullptr;
        }
    }
    ...
    if (flag_MiscComment) {
        g_game->printout(OFFICER_MEDICAL, miscComment, GREEN, 10000);
