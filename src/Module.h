...
class Module {
  public:
     ...
     * allegro.h (for example, ALLEGRO_KEY_SPACE). make sure to call the base
     * class impl.
     */
    ...
    /**
     * JH 01-Feb-07
     * Added to handle key press events without the engine trapping
     * press/release events. This will provide support for multiple and repeat
     * key presses.
     */
     ...
     * then use RTTI (dynamic_cast) to see which specific kind of event is being
     * sent
     */
  ...
  protected:
    // nybble moved these here so derived classes can know where they are in
    // relation to their parent position relative to parent
    int m_x;
    int m_y;

  private:
    // child modules (used when this module is a game mode root, not used when
